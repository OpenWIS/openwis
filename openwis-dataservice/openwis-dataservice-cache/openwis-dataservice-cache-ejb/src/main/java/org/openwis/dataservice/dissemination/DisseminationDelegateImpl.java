...
public class DisseminationDelegateImpl implements ConfigurationInfo, DisseminationDelegate {
    ...
    private static final String STAGING_POST_MAIL_TEMPLATE = "dissemination-sp-mail-template.html";
    ...
    private boolean disseminateViaSP(DisseminationJob dissJob, boolean startPrimary) {
        ...
        String content = getStagingPostMessageContent(processedRequest, dissJob.getFileURI());
    ...
    private String getStagingPostMessageContent(ProcessedRequest processedRequest, String fileUrl) {
        ...
        data.put("title", processedRequest.getRequest().getProductMetadata().getTitle());
        data.put("urn", processedRequest.getRequest().getProductMetadata().getUrn());
        data.put("requestID", processedRequest.getRequest().getId());
        data.put("isSubscription", !processedRequest.getRequest().getRequestType().equals("ADHOC"));
        data.put("downloadLink", stagingPostUrl + fileUrl);
        Context context = new Context(new Locale("en"), data);

        ClassLoader classLoader = getClass().getClassLoader();
        String template = new BufferedReader(
                new InputStreamReader(classLoader.getResourceAsStream("templates/" + STAGING_POST_MAIL_TEMPLATE), StandardCharsets.UTF_8))
                .lines()
