package org.openwis.harness.samples.fs.subselectionparameters;
...
public class SubSelectionParametersImpl implements SubSelectionParameters {
   ...
   Parameters getSubSelectionParametersForRequest(
      ...
      return getParameters(new Locale(lang), false);
   }
   ...
   Parameters getSubSelectionParametersForSubscription(
         @WebParam(name = "productMetadataURN") String urn, @WebParam(name = "lang") String lang) {
      return getParameters(new Locale(lang), true);
   }
    ...
    */
   private Parameters getParameters(Locale locale, boolean subscription) {
      ...
      // Fill result
      if (subscription) {
         result.setStartParameter(tps);
      } else {
         result.getParameters().add(dps);
         result.setStartParameter(dps);
      }
