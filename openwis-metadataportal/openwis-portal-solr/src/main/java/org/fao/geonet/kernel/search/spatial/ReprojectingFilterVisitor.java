 ...
 */
 ...
 // Added to GeoNetwork because of a bug around line 110 - needs to be fixed
 // in Geotools 2.6.x
 ...
 *
 * @source $URL: http://svn.osgeo.org/geotools/tags/2.6.2/modules/unsupported/wfs/src/main/java/org/geotools/data/wfs/v1_1_0/ReprojectingFilterVisitor.java $
 */
public class ReprojectingFilterVisitor extends DuplicatingFilterVisitor {

   private static Logger logger = LoggerFactory.getLogger(ReprojectingFilterVisitor.class);
        ...
        try {
            ...
            if (crs != null && targetCrs != null && !CRS.equalsIgnoreMetadata(crs, targetCrs)) {
                ...
                maxy = envelope.getMaxY();
								// BUG: There isn't another identifier! sppigot	
								//srs = targetCrs.getIdentifiers().iterator().next().toString();`
                srs = targetCrs.getIdentifiers().toString();
            }
