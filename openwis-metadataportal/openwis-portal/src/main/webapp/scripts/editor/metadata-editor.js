var getGNServiceURL = function(service) {
	return configOptions.locService+"/"+service;
};

var getProxy = function() {
    return "../../proxy?" //FIXME JB
};
...
var Checks = {
	message : Openwis.i18n('Common.Editor.LoseYourChanges'),
...
{
	if(metadata.edit.embedded.doAction !== undefined) {
        metadata.edit.embedded.doAction(action);
    } else {
        setBunload(false);
...
{
    if(metadata.edit.embedded.doTabAction !== undefined) {
...
function showKeywordSelectionPanel(ref, name) {
    if (!keywordsSearch) {
    	...
    	var keywordsSearch = new Openwis.Common.Search.KeywordsSearch({
			keywordsFromTf: "",
			isXML: true,
			listeners: {
				keywordsSelection: function(records) {
	        		...
	        		Ext.each(records, function(item, index) {
						// Format XML
	        			records[index] = item.replace("<?xml version=\"1.0\" encoding=\"UTF-8\"?>","")
							.replace(/\"/g,"&quot;").replace(/\r\n/g,"");
						if (first) {
							xml = records[index];
							first = false;
...
/**
 * Remove linked dataset from service metadata.
 * @param uuid the service metadata uuid
 * @param uuidref the dataset metadata uuid
 */
function removeLinkedServiceMetadata(uuid, uuidref) {
	var serviceUpdateUrl = "xml.metadata.processing?uuid=" + uuid
			+ "&process=update-srv-detachDataset&uuidref=" + uuidref;
	setBunload(false);
	Ext.Ajax.request({
...
/**
 * Remove linked feature metadata.
 * @param parentUuid the parent metadata uuid
 * @param uuid the child metadata uuid
 */
function removeLinkedFeatureMetadata(parentUuid, childUuid) {
	var serviceUpdateUrl = "xml.relation.delete?parentUuid=" + parentUuid
			+ "&childUuid=" + childUuid;
	setBunload(false);
	Ext.Ajax.request({
		url: serviceUpdateUrl,
...
function showLinkedServiceMetadataSelectionPanel(name, serviceUrl, uuid) {
	...
	var linkedMetadataSelectionPanel = new app.LinkedMetadataSelectionPanel({
        ...
        listeners: {
            linkedmetadataselected: function(panel, metadata) {
				var layerName = panel.layerName.getValue();
				...
				if (name=='attachService') {
					...
					Ext.Ajax.request({
						...
						success: function(result, request) {
							...
							// TODO Update current metadata record with ci online resource
...
//			    					"&scopedName=" + layerName);
					...
					var datasetUpdateUrl = "xml.metadata.processing?uuid=" + uuid + 
	    					"&process=update-srv-attachDataset&uuidref=" +
