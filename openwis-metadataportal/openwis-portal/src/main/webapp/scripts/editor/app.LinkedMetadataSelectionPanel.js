...
app.LinkedMetadataSelectionPanel = Ext.extend(Ext.FormPanel, {
    ...
    initComponent: function() {
	            ...
	            listeners: {
	        		...
	        		beforeload: function() {
	        			...
	        			if (this.mode=='attachService') {
	            			var selected = this.linkedMetadataGrid.getSelectionModel().getSelections();
	        	            if (selected == undefined || selected.length == 0 || selected[0].data.uri == '') {
        ...
        // clean data store before displaying the grid
        app.linkedMetadata.linkedMetadataStore.removeAll();
        
        this.linkedMetadataGrid = new Ext.grid.GridPanel({
        ...
        if (this.mode=='attachService' || this.mode=='coupledResource') 
        	this.items = this.getScopedNamePanel(this.linkedMetadataGrid);
        else
        	this.items = this.linkedMetadataGrid;
        ...
        // Disable availability of creating new metadata from here
        //, 
    ...
    getScopedNamePanel: function(grid) {
	    ...
	    var combo = {
	        ...
	        listeners: {
	    		select: function(combo, record, index) {
	    			getLayerName().setValue(combo.getValue());
	    		},
		...
		var panel = {
	        ...
	        border: true,
	        items: [grid, combo, this.getLayerName()]
		};
		return panel;
    },
    ...
    getLayerName: function() {
    	if(!this.layerName) {
			this.layerName = new Ext.form.TextField({
