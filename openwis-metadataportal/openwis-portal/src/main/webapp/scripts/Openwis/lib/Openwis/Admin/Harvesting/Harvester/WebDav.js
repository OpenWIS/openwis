...
Openwis.Admin.Harvesting.Harvester.WebDav = Ext.extend(Ext.Window, {
	
	initComponent: function() {
		Ext.apply(this, 
		{
			title: Openwis.i18n('Harvesting.WebDav.Title'),
			layout: 'fit',
			width:450,
			height:620,
			modal: true,
	...
	getConfigurationOptionsCheckBoxGroup: function() {
		if(!this.configurationOptionsCheckboxGroup) {
			this.configurationOptionsCheckboxGroup = new Ext.form.CheckboxGroup({
				...
				items:
				[
				    {boxLabel: Openwis.i18n('Harvesting.ProviderConfiguration.Options.Recursive'), name: 'recursive', id: 'recursive', checked: true},
				    {boxLabel: Openwis.i18n('Harvesting.ProviderConfiguration.Options.Validate'), name: 'validate', id: 'validate', checked: false}
				]
			});
		}
	...
	getHarvestingTask: function() {
		...
		Ext.each(values, function(item, index, allItems) {
		    if(item.name == 'recursive') {
		        ...
		        task.configuration.validate = true;
		    }
		}, this);
