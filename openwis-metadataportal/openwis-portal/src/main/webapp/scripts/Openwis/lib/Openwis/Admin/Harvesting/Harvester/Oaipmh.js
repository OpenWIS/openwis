...
Openwis.Admin.Harvesting.Harvester.Oaipmh = Ext.extend(Ext.Window, {
	
	initComponent: function() {
		Ext.apply(this, 
		{
			title: Openwis.i18n('Harvesting.Oaipmh.Title'),
			layout: 'fit',
			width:450,
			height:620,
			modal: true,
	...
	getProviderConfigurationOptionsCheckBoxGroup: function() {
		if(!this.providerConfigurationOptionsCheckboxGroup) {
			this.providerConfigurationOptionsCheckboxGroup = new Ext.form.CheckboxGroup({
				...
				items:
				[
				    {boxLabel: Openwis.i18n('Harvesting.ProviderConfiguration.Options.DeletionSupport'), name: 'deletionSupport', id: 'deletionSupport', disabled: true},
				    {boxLabel: Openwis.i18n('Harvesting.ProviderConfiguration.Options.Incremental'), name: 'incremental', id: 'incremental', disabled: true}
				]
			});
		}
    					...
    					new Openwis.Handler.Get({
                			...
                			listeners: {
                				success: function(remoteConfig) {
                					...
                					if(!this.isEdition()) {
                					    this.getProviderConfigurationOptionsCheckBoxGroup().setValue('deletionSupport', remoteConfig.deletionSupport);
