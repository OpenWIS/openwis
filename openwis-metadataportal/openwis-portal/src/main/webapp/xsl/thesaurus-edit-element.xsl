...
function checkForm(form){
	...
	if (checkString(form.prefLab,"<xsl:value-of select="/root/response/prefLab"/>",false)==false){
		return false;		
	};	
...
function submitKeywordForm(){
	...
	if (checkForm(form)){
		var url = form.action + '?' +Form.serialize('keywordForm');	// FIXME
								...
								<xsl:otherwise>
									...
									<select class="md" name="lang" id="lang" type="text">
									   <xsl:for-each select="/root/gui/languages/record">
										   ...
										   <option>
												<xsl:attribute name="selected"><xsl:value-of select="$langId"/></xsl:attribute>
												<xsl:attribute name="value"><xsl:value-of select="$langId"/></xsl:attribute>
												<xsl:value-of select="/root/gui/strings/child::*[name() = $langId]"/>
										   </option>
										   </xsl:if>
									   </xsl:for-each>
								   </select>
