...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template mode="iso19139"
		match="gmd:*[gco:Date|gco:DateTime|gco:Integer|gco:Decimal|gco:Boolean|gco:Real|gco:Measure|gco:Length|gco:Distance|gco:Angle|gco:Scale|gco:RecordType|gmx:MimeFileType]|
			...
			<xsl:when test="$edit=true()">
				<xsl:variable name="text">
					...
					<input type="text" class="md" name="_{$ref}" id="_{$ref}"
						...
						onchange="validateNumber(this,true,true);" value="{gco:Distance}" size="30"
			...
			<xsl:otherwise>
				<xsl:apply-templates mode="simpleElement" select=".">
					...
					<xsl:with-param name="text">
						<xsl:value-of select="gco:Distance"/>&#160;<xsl:value-of
							select="gco:Distance/@uom"/>
					</xsl:with-param>
				</xsl:apply-templates>
			</xsl:otherwise>
		</xsl:choose>
		...
		<xsl:variable name="text">
			...
			<xsl:choose>
				<xsl:when test="$edit=true()">
					...
					<input type="text" name="_{$ref}_uuidref" id="_{$ref}_uuidref"
						value="{./@uuidref}" size="20"
			...
			<xsl:choose>
				<xsl:when test="$edit=true()">
					...
					<input type="checkbox" id="N{$ref}" onchange="buildDuration('{$ref}');">
						<xsl:if test="$NEG!=''"><xsl:attribute name="checked"
								>checked</xsl:attribute></xsl:if>
					</input>
					<label for="N{$ref}"><xsl:value-of select="/root/gui/strings/durationSign"
						/></label><br/>
					<xsl:value-of select="/root/gui/strings/durationNbYears"/><input type="text"
						id="Y{$ref}" class="content"
						value="{substring-before(substring-after($p, 'P'), 'Y')}" size="4"
						onchange="buildDuration('{$ref}');"
						onkeyup="validateNumber(this,true,false);"/>- <xsl:value-of
						select="/root/gui/strings/durationNbMonths"/><input type="text" id="M{$ref}"
						class="content" value="{substring-before(substring-after($p, 'Y'), 'M')}"
						size="4" onchange="buildDuration('{$ref}');"
						onkeyup="validateNumber(this,true,false);"/>- <xsl:value-of
						select="/root/gui/strings/durationNbDays"/><input type="text" id="D{$ref}"
						class="content" value="{substring-before(substring-after($p, 'M'), 'DT')}"
						size="4" onchange="buildDuration('{$ref}');"
						onkeyup="validateNumber(this,true,false);"/><br/>
					<xsl:value-of select="/root/gui/strings/durationNbHours"/><input type="text"
						id="H{$ref}" class="content"
						value="{substring-before(substring-after($p, 'DT'), 'H')}" size="4"
						onchange="buildDuration('{$ref}');"
						onkeyup="validateNumber(this,true,false);"/>- <xsl:value-of
						select="/root/gui/strings/durationNbMinutes"/><input type="text"
						id="MI{$ref}" class="content"
						value="{substring-before(substring-after($p, 'H'), 'M')}" size="4"
						onchange="buildDuration('{$ref}');"
						onkeyup="validateNumber(this,true,false);"/>- <xsl:value-of
						select="/root/gui/strings/durationNbSeconds"/><input type="text"
						id="S{$ref}" class="content"
						value="{substring-before(substring-after(substring-after($p,'M' ),'M' ), 'S')}"
						size="4" onchange="buildDuration('{$ref}');"
						onkeyup="validateNumber(this,true,true);"/><br/>
				...
				<xsl:otherwise>
					...
					<xsl:text> </xsl:text>
					<xsl:value-of select="/root/gui/strings/durationYears"/>
					<xsl:text>  </xsl:text>
					<xsl:value-of select="$M"/>
					<xsl:text> </xsl:text>
					<xsl:value-of select="/root/gui/strings/durationMonths"/>
					<xsl:text>  </xsl:text>
					<xsl:value-of select="$D"/>
					<xsl:text> </xsl:text>
					<xsl:value-of select="/root/gui/strings/durationDays"/>
					<xsl:text> / </xsl:text>
					<xsl:value-of select="$H"/>
					<xsl:text> </xsl:text>
					<xsl:value-of select="/root/gui/strings/durationHours"/>
					<xsl:text>  </xsl:text>
					<xsl:value-of select="$MI"/>
					<xsl:text> </xsl:text>
					<xsl:value-of select="/root/gui/strings/durationMinutes"/>
					<xsl:text>  </xsl:text>
					<xsl:value-of select="$S"/>
					<xsl:text> </xsl:text>
					<xsl:value-of select="/root/gui/strings/durationSeconds"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:apply-templates mode="simpleElement" select=".">
	...
	<xsl:template name="iso19139String">
		...
		<!-- FIXME Overflow in Edit page (set to 30) -->
			...
			<xsl:when test="$edit">
				<xsl:apply-templates mode="simpleElement" select=".">
					...
					<xsl:with-param name="title"
						select="/root/gui/iso19139/element[@name='gml:timeInterval']/label"/>
				</xsl:apply-templates>
			</xsl:when>
			<xsl:otherwise>
				<xsl:variable name="text">
					<xsl:choose>
						<xsl:when test="@radix and @factor"><xsl:value-of
								select=". * @factor div @radix"/>&#160;<xsl:value-of select="@unit"
							/></xsl:when>
						<xsl:when test="@factor"><xsl:value-of select=". * @factor"
								/>&#160;<xsl:value-of select="@unit"/></xsl:when>
						<xsl:when test="@radix"><xsl:value-of select=". div @radix"
								/>&#160;<xsl:value-of select="@unit"/></xsl:when>
				...
				<xsl:choose>
					<xsl:when test="$edit">
						...
						<xsl:call-template name="simpleElementGui">
							...
							<xsl:with-param name="text">
								<button class="content"
									onclick="startFileUpload({//geonet:info/id}, '{$ref}');"
									type="button">
						...
						<xsl:call-template name="simpleElementGui">
							...
							<xsl:with-param name="text">
								<input id="_{$ref}" class="md" type="text" name="_{$ref}"
									value="{$value}" size="40"/>
							</xsl:with-param>
	...
	<xsl:template mode="iso19139" match="gmd:LanguageCode" priority="2">
		...
		<xsl:choose>
			<xsl:when test="$edit=true()">
				<select class="md" name="_{geonet:element/@ref}_codeListValue" size="1">
					<option name=""/>
	...
	and iso code.-->
	<xsl:template mode="iso19139" match="gmd:PT_Locale/@id" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
				...
				<xsl:choose>
						...
						<select class="md" name="_{../geonet:element/@ref}_{name(.)}"
							...
							<xsl:if test="../../geonet:element/@min='1' and $edit">
								<xsl:attribute name="onchange"
									>validateNonEmpty(this);</xsl:attribute>
							</xsl:if>
					...
					<xsl:otherwise>
						...
						<xsl:if test="normalize-space($value)!=''">
							...
							<xsl:value-of
								select="concat(': ',$codelist/entry[code = $value]/description)"/>
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
			...
			<xsl:with-param name="text">
				<xsl:choose>
					<xsl:when test="normalize-space(gco:*)=''">
						<span class="info"> - <xsl:value-of select="/root/gui/strings/setOnSave"/> -
						</span>
					</xsl:when>
					<xsl:otherwise>
	...
	<xsl:template mode="iso19139"
		...
		<xsl:variable name="text">
			...
			<input class="md" type="text" name="_{$ref}" value="{gco:CharacterString/text()}"
				size="40"/>
	...
	<xsl:template mode="iso19139"
		match="gmd:dateTime|gmd:usageDateTime|gmd:plannedAvailableDateTime" priority="2">
		<xsl:param name="schema"/>
		<xsl:param name="edit"/>
	...
	<xsl:template mode="iso19139"
		...
		<xsl:for-each select="*">
			<xsl:choose>
				<xsl:when test="$edit=true() and (name(.)='gml:beginPosition' 
					or name(.)='gml:endPosition' or name(.)='gml:timePosition')">
					<xsl:apply-templates mode="simpleElement" select=".">
						<xsl:with-param name="schema" select="$schema"/>
						<xsl:with-param name="edit" select="$edit"/>
						<xsl:with-param name="text">
	...
	<xsl:template name="isotabs">
				...
				<xsl:with-param name="title">
					<xsl:choose>
						<xsl:when test="$dataset=true()">
							<xsl:value-of
								select="/root/gui/iso19139/element[@name='gmd:MD_DataIdentification']/label"
							/>
						</xsl:when>
						<xsl:when
							test="local-name(.)='SV_ServiceIdentification' or @gco:isoType='srv:SV_ServiceIdentification'">
							<xsl:value-of
								select="/root/gui/iso19139/element[@name='srv:SV_ServiceIdentification']/label"
							/>
						</xsl:when>
						<xsl:otherwise>
				...
				<xsl:with-param name="content">
					...
					<xsl:if test="$dataset">
						<xsl:for-each select="gmd:extent/gmd:EX_Extent">
							<xsl:call-template name="complexElementGuiWrapper">
								<xsl:with-param name="title"
									select="/root/gui/iso19139/element[@name='gmd:EX_Extent']/label"/>
								<xsl:with-param name="content">
		...
		<xsl:if test="$core and $dataset">
			...
			<xsl:call-template name="complexElementGuiWrapper">
				<xsl:with-param name="title"
					select="/root/gui/iso19139/element[@name='gmd:LI_Lineage']/label"/>
				<xsl:with-param name="id"
					select="generate-id(/root/gui/iso19139/element[@name='gmd:LI_Lineage']/label)"/>
				<xsl:with-param name="content">
			...
			<xsl:call-template name="complexElementGuiWrapper">
				<xsl:with-param name="title"
					select="/root/gui/iso19139/element[@name='gmd:referenceSystemInfo']/label"/>
				<xsl:with-param name="id"
					select="generate-id(/root/gui/iso19139/element[@name='gmd:referenceSystemInfo']/label)"/>
				<xsl:with-param name="content">
			...
			<xsl:call-template name="complexElementGuiWrapper">
				<xsl:with-param name="title"
					select="/root/gui/iso19139/element[@name='gmd:distributionInfo']/label"/>
				<xsl:with-param name="id"
					select="generate-id(/root/gui/iso19139/element[@name='gmd:distributionInfo']/label)"/>
				<xsl:with-param name="content">
		...
		<xsl:call-template name="complexElementGuiWrapper">
			<xsl:with-param name="title"
				select="/root/gui/iso19139/element[@name='gmd:MD_Metadata']/label"/>
			<xsl:with-param name="id"
				select="generate-id(/root/gui/iso19139/element[@name='gmd:MD_Metadata']/label)"/>
			<xsl:with-param name="content">
				...
				<xsl:for-each select="gmd:contact">
					...
					<xsl:call-template name="complexElementGuiWrapper">
						<xsl:with-param name="title"
							select="/root/gui/iso19139/element[@name='gmd:contact']/label"/>
						<xsl:with-param name="content">
				...
				<xsl:choose>
					<xsl:when test="$edit">
						<xsl:call-template name="complexElementGui">
							<xsl:with-param name="title" select="$title"/>
							<xsl:with-param name="content">
								<span class="missing"> - <xsl:value-of
										select="/root/gui/strings/missingSeeTab"/> "<xsl:value-of
										select="$group"/>" - </span>
							</xsl:with-param>
							<xsl:with-param name="helpLink">
								<xsl:call-template name="getHelpLink">
					...
					<xsl:otherwise>
						<xsl:call-template name="complexElementGui">
							...
							<xsl:with-param name="content">
								<span class="missing"> - <xsl:value-of
										select="/root/gui/strings/missing"/> - </span>
							</xsl:with-param>
							<xsl:with-param name="schema" select="$schema"/>
							<xsl:with-param name="id" select="$id"/>
						</xsl:call-template>
	...
	<xsl:template mode="iso19139" match="gmd:transferOptions">
		...
		<xsl:if test="$edit=false()">
			<xsl:if
				test="count(gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource/gmd:protocol/gco:CharacterString[contains(string(.),'download')])>1 and
	...
	<xsl:template mode="iso19139EditOnlineRes" match="*">
		...
		<xsl:apply-templates mode="complexElement" select=".">
			...
			<xsl:with-param name="content">
				...
				<xsl:choose>
					<xsl:when
						test="string(gmd:protocol/gco:CharacterString)='WWW:DOWNLOAD-1.0-http--download' and string(gmd:name/gco:CharacterString|gmd:name/gmx:MimeFileType)!=''">
			...
			<xsl:when
				...
				<xsl:apply-templates mode="simpleElement" select=".">
					...
					<xsl:with-param name="text">
						...
						-->
						<a
							href="javascript:addWMSLayer([['{$name}','{$linkage}','{$name}','{$metadata_id}']])"
						...
						Server: <xsl:value-of select="$linkage"/> ) </xsl:with-param>
				</xsl:apply-templates>
	...
	<xsl:template mode="iso19139"
		...
		<xsl:choose>
			...
			<xsl:when
				test="string(//geonet:info/dynamic)='true' and string($linkage)!='' and string($name)!=''">
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="title" select="/root/gui/strings/interactiveMap"/>
					<xsl:with-param name="text">
...
-->
						<a href="javascript:runIM_addService('{$linkage}','{$name}',1)"
							title="{/root/strings/interactiveMap}">
							<xsl:choose>
								<xsl:when test="string($description)!=''">
									<xsl:value-of select="$description"/>
				...
				<xsl:call-template name="simpleElementGui">
					...
					<xsl:with-param name="text">
						...
						<select id="s_{$ref}" name="s_{$ref}" size="1"
							onchange="checkForFileUpload('{$fref}', '{$ref}');" class="md">
							<xsl:if test="$value=''">
								<option value=""/>
							</xsl:if>
		...
		<xsl:choose>
			<xsl:when test="$edit=true()">
				...
				<xsl:variable name="pref"
					select="../gmd:protocol/gco:CharacterString/geonet:element/@ref"/>
				<xsl:variable name="ref"
				...
				<xsl:variable name="button"
					select="starts-with($protocol,'WWW:DOWNLOAD') and contains($protocol,'http') and normalize-space($value)=''"/>
				...
				<xsl:call-template name="simpleElementGui">
					...
					<xsl:with-param name="text">
						<button class="content"
							onclick="startFileUpload({//geonet:info/id}, '{$ref}');" type="button">
							<xsl:value-of select="/root/gui/strings/insertFileMode"/>
						</button>
					</xsl:with-param>
					<xsl:with-param name="id" select="concat('db_',$ref)"/>
	...
	<xsl:template name="iso19139Brief">
		<metadata>
			...
			<xsl:apply-templates mode="briefster"
				select="gmd:identificationInfo/gmd:MD_DataIdentification|gmd:identificationInfo/*[@gco:isoType='gmd:MD_DataIdentification']|gmd:identificationInfo/srv:SV_ServiceIdentification">
				<xsl:with-param name="id" select="$id"/>
				<xsl:with-param name="langId" select="$langId"/>
			</xsl:apply-templates>
			...
			<xsl:for-each
				select="gmd:distributionInfo/gmd:MD_Distribution/gmd:distributor/gmd:MD_Distributor/gmd:distributorTransferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource">
				<xsl:variable name="protocol" select="gmd:protocol/gco:CharacterString"/>
				<xsl:variable name="linkage" select="normalize-space(gmd:linkage/gmd:URL)"/>

				<xsl:if
					test="string($linkage)!='' and (contains($linkage,'/retrieve/subscribe/') or contains($linkage,'/retrieve/request/'))">
					...
					<xsl:variable name="action">
						<xsl:choose>
							...
							<!-- TODO use CI_OnlineFunctionCode -->
						</xsl:choose>
					</xsl:variable>
			...
			<xsl:for-each
				...
				<xsl:variable name="mimeType"
					select="normalize-space(gmd:name/gmx:MimeFileType/@type)"/>
			...
			<geonet:info>
				...
				This type of categories could be added to index also in order
				to be queriable. 
	...
	<xsl:template mode="briefster" match="*">
		...
		<xsl:for-each select="*/gmd:MD_LegalConstraints/*">
			...
			<!-- OpenWIS GTS Category -->
			<isGlobal>
				<xsl:if
					test="name(.)='gmd:useLimitation' and 
	...
	<xsl:template mode="iso19139" match="gmd:graphicOverview" priority="2">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:apply-templates mode="simpleElement" select=".">
					<xsl:with-param name="schema" select="$schema"/>
					<xsl:with-param name="text">&#160; <xsl:variable name="langId">
							<xsl:call-template name="getLangId">
								<xsl:with-param name="langGui" select="/root/gui/language"/>
								<xsl:with-param name="md"
									select="ancestor-or-self::*[name(.)='gmd:MD_Metadata' or @gco:isoType='gmd:MD_Metadata']"
						...
						<xsl:variable name="imageTitle">
							<xsl:choose>
								<xsl:when
									test="gmd:MD_BrowseGraphic/gmd:fileDescription/gco:CharacterString
						...
						<xsl:choose>
							...
							<xsl:when
								...
								<div class="thumbnail">
									<a
										href="{gmd:MD_BrowseGraphic/gmd:fileName/gco:CharacterString}"
										target="thumbnail-view">
										<xsl:attribute name="alt"><xsl:value-of select="$imageTitle"
											/></xsl:attribute>
										<xsl:attribute name="title"><xsl:value-of
												select="$imageTitle"/></xsl:attribute>
										<img class="thumbnail"
									...
									<br/>
									<span class="thumbnail"><xsl:value-of select="$imageTitle"
										/></span>
								</div>
							</xsl:when>
							<xsl:otherwise>
		...
		<xsl:variable name="rows">
			<xsl:choose>
				...
				<xsl:when
					...
					or name(.)='gmd:evaluationMethodDescription'
					or name(.)='gmd:measureDescription'">3</xsl:when>
							...
							<xsl:choose>
								...
								is set in gmd:locale. -->
									<select class="md lang_selector" name="localization"
										...
										<xsl:attribute name="style">
											<xsl:choose>
												<xsl:when
												test="count($ptFreeTextTree//gmd:LocalisedCharacterString)=0"
										...
										<xsl:choose>
											...
											<xsl:otherwise>
												<xsl:for-each
												select="$ptFreeTextTree//gmd:LocalisedCharacterString">
												<option value="_{geonet:element/@ref}"
												code="{substring-after(@locale, '#')}">
												<xsl:value-of select="@language"/>
												</option>
								...
								-->
									<xsl:if test="/root/gui/config/editor-google-translate = 1">
										<xsl:text> </xsl:text>
										<a
											href="javascript:googleTranslate('{$mainLanguageRef}',
											'{$suggestionDiv}',
											null,
										...
										<br/>
										<div
											id="suggestion_{gco:CharacterString/geonet:element/@ref|
										gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString[@locale=$mainLangId]/geonet:element/@ref}"
											style="display:none;" class="suggestion"
											alt="{/root/gui/strings/translateWithGoogle}"
											title="{/root/gui/strings/translateWithGoogle}"/>
									</xsl:if>
								</xsl:when>
	...
	<xsl:template name="PT_FreeText_Tree">
		<xsl:variable name="mainLang" select="string(/root/*/gmd:language/gco:CharacterString)"/>
		...
		<xsl:for-each select="$languages">
			<xsl:variable name="langId" select="concat('&#35;',string(../../../@id))"/>
			...
			<gmd:PT_FreeText>
				<gmd:textGroup>
					<gmd:LocalisedCharacterString locale="{$langId}" code="{$code}"
						language="{$language}">
						<xsl:value-of
   ...
   <xsl:template mode="iso19139" match="*[gmx:Anchor]">
      <xsl:param name="schema" />
      <xsl:param name="edit" />
      <xsl:variable name="linkage" select="gmx:Anchor/@xlink:href" />
      <xsl:variable name="name" select="gmx:Anchor" />
      <xsl:choose>
         <xsl:when test="$edit=true()">
            ...
            <xsl:apply-templates mode="simpleElement"
               ...
               <xsl:with-param name="text">
                  <xsl:call-template name="Link">
                     <xsl:with-param name="url" select="$linkage" />
   ...
   <xsl:template name="Link">
      ...
      <u onMouseOver="style.cursor='pointer'"
         onClick="JavaScript:window.open('{$url}','XLinkWindow','width=600,height=400,left=50,top=50,scrollbars=yes,resizable=yes')">
         <xsl:choose>
            <xsl:when test="string($linkname)!=''">
