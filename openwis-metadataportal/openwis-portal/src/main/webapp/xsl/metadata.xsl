...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template mode="elementEP" match="geonet:child">
		...
		<xsl:variable name="exception"
			select="../gmd:graphicOverview[gmd:MD_BrowseGraphic/gmd:fileDescription/gco:CharacterString='thumbnail' or gmd:MD_BrowseGraphic/gmd:fileDescription/gco:CharacterString='large_thumbnail']"/>
				...
				<xsl:call-template name="editAttribute">
					...
					<xsl:with-param name="removeLink">
						<xsl:if test="count(../geonet:attribute[@name=$name and @del])!=0">
							<xsl:value-of
								select="concat('doRemoveAttributeAction(',$apos,'/metadata.attr.delete',$apos,',',$apos,$id,$apos,','
								,$apos,../geonet:element/@ref,$apos,',',$apos,$id,$apos,',',$apos,$apos,');')"
							/>
						</xsl:if>
					</xsl:with-param>
				</xsl:call-template>
			...
			<xsl:call-template name="editAttribute">
				...
				<xsl:with-param name="addLink">
					<xsl:if test="@add='true'">
						<xsl:value-of
							select="concat('doNewAttributeAction(',$apos,'/metadata.elem.add',$apos,',',../geonet:element/@ref,',',$apos,@name,$apos,',',
			...
			<ul>
				<xsl:choose>
					...
					<xsl:when test="@geonet:xsderror">
						<li>
							<xsl:value-of
								select="concat(/root/gui/strings/xsdError,': ',@geonet:xsderror)"/>
						</li>
					</xsl:when>
				...
				<xsl:variable name="nb">
					<xsl:if test="name($nextBrother)='geonet:child'">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:if
									test="concat($nextBrother/@prefix,':',$nextBrother/@name)=$name">
									<xsl:copy-of select="$nextBrother"/>
								</xsl:if>
							</xsl:otherwise>
						</xsl:choose>
			...
			<th class="md" width="20%" valign="top">
				...
				<xsl:choose>
					<xsl:when test="$helpLink!=''">
						<span id="stip.{$helpLink}|{$id}" onclick="toolTip(this.id);"
							class="content" style="cursor:help;">
								...
								<span>
									<div onclick="toggleFieldset(this, $('toggled{$id}'));">
										<xsl:attribute name="class">
											<xsl:choose>
												<xsl:when test="$visibleAttributes">downBt
												Bt</xsl:when>
			...
			<xsl:choose>
				<xsl:when test="starts-with($schema,'iso19139')">
					...
					<xsl:variable name="schematitle"
						select="string(/root/gui/*[name(.)=$schema]/element[@name=$name and not(@context)]/label)"/>

					<xsl:choose>
						<xsl:when
							test="normalize-space($schematitle)='' and
						...
						<xsl:when
							test="normalize-space($schematitleWithContext)='' and
										normalize-space($schematitleWithContextIso)=''">
			...
			<xsl:when test="$edit=true() and $rows=1">
				<xsl:choose>
					<xsl:when
						...
						<input class="md" type="text" id="_{geonet:element/@ref}"
							name="_{geonet:element/@ref}" value="{text()}" size="{$cols}">
							<xsl:if test="$visible = 'false'">
								<xsl:attribute name="style">display:none;</xsl:attribute>
							</xsl:if>
							<xsl:if test="$isXLinked">
					...
					<xsl:when test="name(.)='gco:Boolean'">
						<input type="hidden" name="_{geonet:element/@ref}"
							id="_{geonet:element/@ref}" value="{.}">
							<xsl:if test="$isXLinked">
								<xsl:attribute name="disabled">disabled</xsl:attribute>
							</xsl:if>
							<xsl:choose>
						...
						<xsl:choose>
							<xsl:when test="text()='true' or text()='1'">
								<input class="md" type="checkbox"
									id="_{geonet:element/@ref}_checkbox"
						...
						<input class="md" type="{$input_type}" value="{text()}" size="{$cols}">
							...
							<xsl:choose>
								<xsl:when test="$no_name=false()">
									<xsl:attribute name="name">_<xsl:value-of
											select="geonet:element/@ref"/></xsl:attribute>
									<xsl:attribute name="id">_<xsl:value-of
											select="geonet:element/@ref"/></xsl:attribute>
								</xsl:when>
								<xsl:otherwise>
							...
							<xsl:choose>
								...
								<xsl:when
									...
									<xsl:choose>
										<xsl:when test="name(.)='gco:Integer'">
											<xsl:attribute name="onkeyup">validateNumber(this,
												<xsl:value-of select="not($mandatory)"/>,
												false);</xsl:attribute>
										</xsl:when>
										<xsl:otherwise>
											<xsl:attribute name="onkeyup">validateNumber(this,
								...
								<xsl:when test="$mandatory
									and $edit">
									<xsl:attribute name="onkeyup"> validateNonEmpty(this);
									</xsl:attribute>
								</xsl:when>
								<!-- Custom validator -->
				...
				<select class="md" name="_{../geonet:element/@ref}_{$updatename}" size="1">
					...
					<xsl:for-each select="../geonet:attribute/geonet:text">
						<option>
							...
							<xsl:variable name="label"
								select="/root/gui/*[name(.)=$schema]/codelist[@name = $parent]/entry[code=$choiceValue]/label"/>
							<xsl:choose>
								<xsl:when test="$label">
									<xsl:value-of select="$label"/>
								</xsl:when>
	...
	<xsl:template name="getHelpLink">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of
					select="concat($schema,'|', $name ,'|', name(parent::node()) ,'|', ../@gco:isoType)"
				/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="helper">
		...
		<xsl:variable name="helper">
			<xsl:choose>
				<xsl:when
					test="starts-with($schema,'iso19139') and not(/root/gui/*[name(.)=$schema]/element[@name = $parentName]/helper)">
				...
				<xsl:otherwise>
					<xsl:copy-of
						select="/root/gui/*[name(.)=$schema]/element[@name = $parentName]/helper/*"
					/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:if test="normalize-space($helper)!=''">
			...
			<select
				onchange="$('_{$refId}').value=this.options[this.selectedIndex].value; if ($('_{$refId}').onkeyup) $('_{$refId}').onkeyup();"
				class="md">
				<option/>
