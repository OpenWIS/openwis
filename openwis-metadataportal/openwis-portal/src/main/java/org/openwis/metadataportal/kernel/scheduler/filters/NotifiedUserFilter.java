...
/**
 * Filter the users which are not notified yet.
...
public class NotifiedUserFilter implements AccountFilter {
     ...
     */
    private UserLogDTO getLastNotification(List<UserLogDTO> logs, User user) {
        List<UserLogDTO> userLog = logs.stream()
                .filter(l -> l.getUsername().equals(user.getEmailContact()))
    ...
    private Comparator<Timestamp> dateComparator = (timestamp, t1) -> {
