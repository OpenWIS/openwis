...
public class OpenWisInit extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        // get the token from session and remove it. one time use only
        final Optional<String> sessionInitToken = Optional.ofNullable((String) request.getSession().getAttribute("initToken"));

        Optional<String> openWISInitToken = getInitTokenFromCookie(request);
        if (!openWISInitToken.isPresent() || !sessionInitToken.isPresent()) {
            Log.error(LoginConstants.LOG, LoginConstants.ERROR_AUTH_REQUEST + "No sessionInitToken present");
            forwardError(request, response, "Error during login init process");
        }

        if (!sessionInitToken.get().equals(openWISInitToken.get())) {
            Log.error(LoginConstants.LOG, LoginConstants.ERROR_AUTH_REQUEST + "Wrong openWisInitToken used");
    ...
    private void authenticateOrRedirect(boolean isConnectingToIDP, HttpServletRequest request,
        ...
        if (!isConnectingToIDP) {
            if ((idpEntityID == null) || (idpEntityID.length() == 0)) {
                ...
                request.getSession().removeAttribute("initToken");
                SPSSOFederate
    ...
    public Optional<String> getInitTokenFromCookie(HttpServletRequest request) {
        String key = "OpenWISInitToken";
