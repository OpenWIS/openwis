 ...
 */
public class DeleteMetadataTask extends TimerTask implements Runnable {
    ...
    */
   public DeleteMetadataTask(ISearchManager sm, Collection<IndexableElement> toIndex) {
      super();
      this.sm = sm;
   ...
   public void run() {
      try {
         if (Log.isInfo(Geonet.INDEX_ENGINE)) {
            Log.info(Geonet.INDEX_ENGINE, "Going to delete some metadata in the index: " + toIndex);
         }
         sm.delete(toIndex);
