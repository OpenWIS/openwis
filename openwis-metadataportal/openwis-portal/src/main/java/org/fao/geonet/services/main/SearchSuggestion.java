...
public class SearchSuggestion implements Service {
   ...
   public void init(String appPath, ServiceConfig config) throws Exception {
      ...
      _maxNumberOfTerms = Integer.valueOf(config.getValue("max_number_of_terms"));
   ...
   public Element exec(Element params, ServiceContext context) throws Exception {
      ...
      List<TermFrequency> termList = sm.getTermsFrequency(IndexField.getField(fieldName),
            searchValue, _maxNumberOfTerms, _threshold);
      Collections.sort(termList);
      Collections.reverse(termList);

      Iterator<TermFrequency> iterator = termList.iterator();
      while (iterator.hasNext()) {
         TermFrequency freq = iterator.next();
         suggestions.addContent(new Element("item").setAttribute("term", freq.getTerm())
               .setAttribute("freq", String.valueOf(freq.getFrequency())));
      }
