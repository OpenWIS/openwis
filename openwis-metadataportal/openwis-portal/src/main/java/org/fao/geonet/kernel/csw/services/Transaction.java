...
public class Transaction extends AbstractOperation implements CatalogService {
            ...
            if (transactionType.equals("insert") || transactionType.equals("update")
               ...
               // Update
                  ...
                  try {
                     while (inIt.hasNext()) {
                        Element metadata = (Element) inIt.next().clone();
                        if (!metadata.getName().equals("Constraint")
                              && !metadata.getNamespace().equals(Csw.NAMESPACE_CSW)) {
                           boolean updateSuccess = updateTransaction(transRequest, metadata,
                                 context);
                           if (updateSuccess) {
                              totalUpdated++;
                           }
    ...
    * @throws Exception
    */
   private boolean insertTransaction(Element xml, List<String> fileIds, ServiceContext context)
         throws Exception {
      GeonetContext gc = (GeonetContext) context.getHandlerContext(Geonet.CONTEXT_NAME);
      DataManager dataMan = gc.getDataManager();
      ...
      // TODO handle category if requested
      ...
      if (!profile.equals(Geonet.Profile.EDITOR) && !profile.equals(Geonet.Profile.OPERATOR)
            && !profile.equals(Geonet.Profile.ACCESS_ADMIN)
      ...
      String id = dataMan.insertMetadataExt(dbms, schema, xml, source, createDate, changeDate,
            uuid, username);
    ...
    * @throws Exception
    */
   private boolean updateTransaction(Element request, Element xml, ServiceContext context)
         throws Exception {
      GeonetContext gc = (GeonetContext) context.getHandlerContext(Geonet.CONTEXT_NAME);
      DataManager dataMan = gc.getDataManager();
    ...
    */
   @SuppressWarnings("unchecked")
   private List<Element> getResultsFromConstraints(ServiceContext context, Element constr)
         throws CatalogException {
