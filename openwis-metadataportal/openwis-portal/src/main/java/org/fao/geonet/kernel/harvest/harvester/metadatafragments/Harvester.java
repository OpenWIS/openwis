...
class Harvester {
   ...
   public MetadataFragmentsResult harvest() throws Exception {
      ...
      log.debug("  - Removing old metadata fragments before update with harvestUUid of "
            + params.uuid);
      ...
      if (!buildRecords) {
         for (String uuid : localUuids.getUUIDs()) {
            String isTemplate = localUuids.getTemplate(uuid);
            log.debug("  - isTemplate " + isTemplate + " - " + uuid);
            if (!isTemplate.equals("s")) {
               log.debug("  - indexing " + uuid);
   ...
   private void addFragments(List<Element> fragments) throws Exception {
      ...
      for (Element fragment : fragments) {
         ...
         if (schema == null) {
            ...
            String id = dataMan.insertHarvestedMetadata(dbms, schema, md, params.uuid,
                  df.format(date), df.format(date), uuid, null, null,
      ...
      if (buildRecords && templateForLinks != null && matchFragment) {
         ...
         categId.add(params.recordsCategory);
         // Category may be null
