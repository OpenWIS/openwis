...
public class XslProcessing implements Service {
   ...
   public Element exec(Element params, ServiceContext context) throws Exception {
      ...
      try {
         Element processedMetadata = process(id, process, _appPath, params, context, metadata,
               notFound, notOwner, notProcessFound, false);
         if (processedMetadata == null) {
            throw new BadParameterEx("Processing failed", "Not found:" + notFound.size()
                  + ", Not owner:" + notOwner.size() + ", No process found:"
                  + notProcessFound.size() + ".");
         }
      } catch (Exception e) {
         throw e;
      }
    ...
    * @throws Exception
    */
   public static Element process(String id, String process, String appPath, Element params,
         ServiceContext context, Set<Integer> metadata, Set<Integer> notFound,
         Set<Integer> notOwner, Set<Integer> notProcessFound, boolean useIndexGroup)
         throws Exception {
      ...
      if (info == null) {
         ...
         String filePath = appPath + "xml/schemas/" + schema + "/process/" + process + ".xsl";
         ...
         dataMan.updateMetadata(context.getUserSession(), dbms, id, processedMetadata, false,
               null, context.getLanguage());
