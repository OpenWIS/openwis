...
public class ServiceManager {
    ...
    public void setPortalType(String p) {
        portalType = p;
    }
    ...
    public void addService(String pack, Element srv) throws Exception {
        ...
        //--parse csrf class if any
        Element csrfElement = srv.getChild(ConfigFile.Service.Child.CSRF);
        if (csrfElement != null) {
            try {
                Csrf csrfClass = (Csrf) Class.forName(csrfElement.getAttributeValue(ConfigFile.Class.Attr.NAME)).newInstance();
                si.setCsrfObject(csrfClass);
            } catch (ClassNotFoundException e) {
                Log.error(Log.ENGINE, "CSRF class not found: " + csrfElement.getAttributeValue(ConfigFile.Class.Attr.NAME));
            }
        }
    }
    ...
    public void dispatch(ServiceRequest req, final UserSession session) {
        ...
        try {
            while (true) {
                ...
                // Check content type header. If the service was defined without contentType attribute accept everything, otherwise check if the content type is allowed
                ...
                if (contentMismatch) {
                    ...
                    throw new BadRequestEx("Content type mismatch");
                }

                // Check CSRF
                // If a service has a CSRF Validator defined then look for a CSRF token in the header and try to validate it
                // against the token from the session. The csrf token had been generated by a CSRF Generator class and it should be
                // found in the session and in the header
                // If CSRF obj is a generator generate the csrf and added to req header
                HttpServiceRequest httpServiceRequest = (HttpServiceRequest) req;
                Csrf csrf = srvInfo.getCsrfObject();
                if (csrf != null) {
                    if (csrf instanceof CsrfValidator) {
                        CsrfValidator crsfValidator = (CsrfValidator)csrf;
                        if ( !csrfTokenValid(crsfValidator, session, httpServiceRequest.getHeaders()) ) {
                            error("CSRF token not valid.");
                            throw new BadRequestEx("CSRF token is not valid");
                        }
                    } else if (csrf instanceof CsrfGenerator) {
                        CsrfGenerator crsfGenerator = (CsrfGenerator) csrf;
                        String token = crsfGenerator.generateCsrfToken();
                        session.getsHttpSession().setAttribute(Jeeves.Elem.CSRF_TOKEN_HEADER, token);
                        httpServiceRequest.addHeader(Jeeves.Elem.CSRF_TOKEN_HEADER, token);
                    }
                }
    ...
    static void error(String message, Throwable t) {
        Log.error(Log.SERVICE, message, t);
    }

    /**
     * Validate the csrf token. The token must be present in the header and session.
     * @param validator csrf validator class
     * @param session user session
     * @param headers request headers
     * @return true if token if valid
     */
    private boolean csrfTokenValid(CsrfValidator validator, UserSession session, Map<String, String> headers) {
        String headerToken = headers.get(Jeeves.Elem.CSRF_TOKEN_HEADER);
        if (headerToken == null) {
            return false;
        }
        return validator.validateCsrfToken(session, headerToken);
    }
}
